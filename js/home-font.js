...
function initScene() {
 ...
 /* ww = canvas.width = section.getBoundingClientRect().width;
  wh = canvas.height = section.getBoundingClientRect().height; */
    ww = canvas.width = window.innerWidth;
  wh = canvas.height = window.innerHeight;

  ctx.clearRect(0, 0, canvas.width, canvas.height);

  var myFontSize = headline.getAttribute("my-font-size")
  ctx.font = 'bold '+myFontSize+'vw "Abril Fatface"';
  ctx.textAlign = "center";
  ctx.fillText(headline.innerHTML, ww / 2, wh / 2.5);
  ...
  for (var i = 0; i < ww; i += Math.round(ww / 500)) {
    for (var j = 0; j < wh; j += Math.round(ww / 600)) {
