...
;(function(window) {
	...
	var body = document.body,
		...
		loaders = [],
		loadingTimeout,
		myAnimations = ['Hapi','Isis','Satet','Atum','Sobek','Seker','Shu'],
		myGrid = document.querySelector('.grid');
	...
	function applys(type) {
		...
		loadingTimeout = setTimeout(function() {
			...
			// Apply effect.
			loaders[currentGrid]._render(type);
			myGrid.style.opacity ='1'
		}, 2000);
	}
	
	var magnifPopup = function() {
		$('.grid__link').magnificPopup({
			type: 'image',
			removalDelay: 300,
	...
	init();
	magnifPopup();
	applys(myAnimations[parseInt(Math.random()*7)]);
